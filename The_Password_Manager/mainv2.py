    ...
    if len(website1) ==0 or len(user_name) == 0 or len(password) == 0: ## No fields empty condition.
        ...
        if messagebox.askokcancel(title = "Confirmation",message = "The details entered:\n"
            ...
            try:
                ...
                with open("data.json", mode="w") as file: ## Creates a file, if it doesn't exist.
    ...
    try:
        with open("data.json", mode="r") as file:
            ...
            if website1 in data1:
                    uname = data1[website1]["email"]
                    pword = data1[website1]["password"]
                    messagebox.showinfo(title="Details",message = f"website: {website1}\n"
                                                                  f"email : {uname}\n"
                                                                  f"password : {pword}")
                    entry1.delete(0,tk.END)
            else:
...
button3 = tk.Button(text = "Search",width = 11, command = check_info)
button3.grid(row = 1, column = 3,columnspan = 1)
